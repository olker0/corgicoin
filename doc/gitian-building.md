...
*Setup instructions for a gitian build of Corgicoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Corgicoin
...
- [Building Corgicoin](#building-corgicoin)
...
Clone the git repositories for corgicoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/corgicoin/corgicoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the corgicoin repository
...
To build Corgicoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the corgicoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/corgicoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/corgicoin/corgicoin
...
For example:
```bash
URL=https://github.com/laanwj/corgicoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit corgicoin=${COMMIT} --url corgicoin=${URL} ../corgicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit corgicoin=${COMMIT} --url corgicoin=${URL} ../corgicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit corgicoin=${COMMIT} --url corgicoin=${URL} ../corgicoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/corgicoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/corgicoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/corgicoin-osx-build.assert
```
...
[corgicoin/gitian.sigs](https://github.com/corgicoin/gitian.sigs/) repository, or if that's not possible create a pull request. 
