...
// Corgicoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateCorgicoinNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetCorgicoinBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, uint256 prevHash)
{
    ...
    if (!consensusParams.fSimplifiedRewards)
    {
        ...
        else if(nHeight < 400000)      
        {
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 5000);
...
int64_t GetCorgicoinDustFee(const std::vector<CTxOut> &vout, CFeeRate &baseFeeRate) {
    int64_t nFee = 0;
